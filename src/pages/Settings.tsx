...
const Settings: React.FC = () => {
  ...
  const [clinicSpecialty, setClinicSpecialty] = useState(() => localStorage.getItem("clinicSpecialty") || "");
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (displayName !== currentUser.displayName) updates.displayName = displayName;
      if (photoFile) {
    ...
    <div className="p-5 sm:p-6 lg:p-8 max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="space-y-6">
        ...
        <Card className="p-6">
          ...
          <p className="text-sm text-slate-500 mb-6">{t("settings.profile_desc")}</p>

          <form onSubmit={handleSave} className="space-y-5">
            <div className="flex items-center gap-5">
              <img
                src={photoPreview || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName || currentUser?.email || "U")}&background=0D9488&color=fff&bold=true`}
                alt="Perfil"
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-slate-400">{t("settings.photo_hint")}</p>
              </div>
            </div>
            ...
            />
            <Input label={t("login.email")} id="email" type="email" value={currentUser?.email || ""} disabled hint={t("settings.email_hint")} />
            ...
            <div className="flex justify-end pt-1">
              <Button type="submit" loading={loading} className="cursor-pointer">
                {loading ? (success ? t("settings.saved") : t("settings.saving")) : t("settings.save_changes")}
              </Button>
            </div>
          </form>
        </Card>
        ...
        <Card className="p-6">
          ...
          <p className="text-sm text-slate-500 mb-6">{t("settings.appearance_desc")}</p>
        ...
        {/* Security */}
        <Card className="p-6">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t("settings.security")}</h2>
          <p className="text-sm text-slate-500 mb-6">{t("settings.security_desc")}</p>

          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{t("login.password")}</p>
        ...
        {/* Clínica & Marca */}
        <Card className="p-6">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t("settings.clinic_brand")}</h2>
          <p className="text-sm text-slate-500 mb-6">{t("settings.clinic_brand_desc")}</p>
          <div className="space-y-4">
            ...
            {clinicSaved && (
              <div className="flex items-center gap-2 text-sm text-sage-700 bg-sage-50 border border-sage-100 rounded-xl p-3">
                <CheckCircleIcon className="w-5 h-5 shrink-0" /> {t("settings.clinic_saved")}
              </div>
            )}
            <div className="flex justify-end">
              <Button type="button" variant="secondary" onClick={handleSaveClinic} className="cursor-pointer">
        ...
        <Card className="p-6">
          ...
          <p className="text-sm text-slate-500 mb-6">{t("settings.notifications_desc")}</p>
          <div className="space-y-4">
            {([
              { key: "notif_evalComplete", label: t("settings.notif_eval"), value: notifEvalComplete, set: setNotifEvalComplete },
              { key: "notif_weightLog", label: t("settings.notif_weight"), value: notifWeightLog, set: setNotifWeightLog },
              { key: "notif_appointment", label: t("settings.notif_appt"), value: notifAppointment, set: setNotifAppointment },
              ...
              <label key={key} className="flex items-center justify-between gap-4 cursor-pointer">
                ...
                <div className="relative shrink-0">
                  <input
                    ...
                    checked={value}
                    onChange={(e) => { set(e.target.checked); handleNotifToggle(key, e.target.checked); }}
                  />
        ...
        {/* Dados & Privacidade */}
        <Card className="p-6">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t("settings.privacy")}</h2>
          <p className="text-sm text-slate-500 mb-6">{t("settings.privacy_desc")}</p>
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-800">
              <div>
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{t("settings.export_data")}</p>
                <p className="text-xs text-slate-500">{t("settings.export_data_desc")}</p>
              </div>
              <Button type="button" variant="secondary" size="sm" className="cursor-pointer" onClick={() => {
                const data = { email: currentUser?.email, displayName: currentUser?.displayName, uid: currentUser?.uid, exportedAt: new Date().toISOString() };
                const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a"); a.href = url; a.download = "meus-dados-isanutri.json"; a.click(); URL.revokeObjectURL(url);
              }}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-800">
              <div>
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{t("settings.privacy_policy")}</p>
                <p className="text-xs text-slate-500">{t("settings.privacy_policy_desc")}</p>
              </div>
              <Button type="button" variant="secondary" size="sm" className="cursor-pointer" onClick={() => window.open("https://isanutri.com/privacidade", "_blank")}>
