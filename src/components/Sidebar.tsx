...
import { firebaseSignOut, auth, getPatients } from "../services/firebaseService";
...
const NavItem: React.FC<NavItemProps> = ({ to, icon, label, badge, onNavigate }) => (
  <NavLink
    to={to}
    onClick={onNavigate}
    className={({ isActive }) =>
        ...
        {badge && (
          <span
            className={`ml-auto text-[11px] font-bold px-2 py-0.5 rounded-full ${
              isActive ? "bg-white/20 text-white" : "bg-sage-100 text-sage-700 dark:bg-sage-500/15 dark:text-sage-300"
            }`}
          >
            {badge}
          </span>
...
const deriveNotifications = (patients: Patient[]): NotifItem[] => {
  ...
  patients.forEach((p) => {
    ...
    (p.selfEvaluations || [])
      .filter((e) => e.status === "completed" && e.completionDate && new Date(e.completionDate) > galaxyDaysAgo)
      .forEach((e) => {
        items.push({
          id: `eval_${e.id}`,
          icon: "📝",
    ...
    (p.weightHistory || [])
      .filter((w) => w.origin === "self_reported" && new Date(w.date) > galaxyDaysAgo)
      .forEach((w) => {
        items.push({
          id: `weight_${p.id}_${w.date}`,
          icon: "⚖️",
          text: i18n.t("notifications.registered_weight", { name, weight: w.weight }),
...
const NotificationBell: React.FC<{ uid: string; onNavigate?: () => void }> = ({ uid, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const unsub = getPatients(uid, (pts) => { setPatients(pts); setLoaded(true); });
    return () => unsub?.();
  }, [open, uid, loaded]);
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 w-80 bg-white dark:bg-slate-850 rounded-2xl shadow-pop border border-slate-200/70 dark:border-slate-700 z-50 animate-scale-in overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
            <p className="font-bold text-slate-800 dark:text-white text-sm">{t("notifications.title")}</p>
            <span className="text-xs text-slate-400">{t("notifications.last_7_days")}</span>
          </div>
          <div className="max-h-72 overflow-y-auto divide-y divide-slate-50 dark:divide-slate-800">
            {!loaded ? (
              <div className="px-4 py-6 text-center text-sm text-slate-400">{t("notifications.loading")}</div>
            ) : notifications.length === 0 ? (
              <div className="px-4 py-8 text-center">
                <p className="text-2xl mb-2">🔕</p>
                <p className="text-sm text-slate-400">{t("notifications.no_activity")}</p>
              </div>
            ) : (
              notifications.map((n) => (
                <button
                  key={n.id}
                  onClick={() => {
                    if (n.patientId) { navigate(`/patients/${n.patientId}`); onNavigate?.(); }
                    setOpen(false);
                  }}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-700 dark:text-slate-200 font-medium leading-snug">{n.text}</p>
...
/* ----------------------------------------------------------------- Global search */
const GlobalSearch: React.FC<{ onNavigate?: () => void }> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const unsub = getPatients(currentUser.uid, (pts) => { setPatients(pts); setLoaded(true); });
    return () => unsub?.();
  }, [currentUser, loaded]);
  ...
  return (
    <div className="relative px-3 mb-2">
      <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
        ...
        />
        {query && (
          <button onClick={() => setQuery("")} className="text-slate-400 hover:text-slate-600 text-xs cursor-pointer">✕</button>
        )}
      </div>
      {results.length > 0 && (
        <div className="absolute left-3 right-3 top-full mt-1 bg-white dark:bg-slate-855 rounded-xl shadow-pop border border-slate-200/70 dark:border-slate-700 z-50 overflow-hidden">
          {results.map((p) => (
            <button
              key={p.id}
              onClick={() => { navigate(`/patients/${p.id}`); setQuery(""); onNavigate?.(); }}
            ...
            >
              <img
                src={p.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(p.firstName)}&background=0D9488&color=fff&bold=true&size=32`}
                alt={p.firstName}
                className="w-7 h-7 rounded-lg object-cover shrink-0"
              />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-slate-800 dark:text-white truncate">{p.firstName} {p.lastName}</p>
  ...
  return (
    <aside className="w-[272px] h-full shrink-0 bg-white dark:bg-slate-850 border-r border-slate-200/70 dark:border-slate-700/60 flex flex-col">
      ...
      <div className="h-16 flex items-center justify-between px-5 border-b border-slate-100 dark:border-slate-800">
        <button onClick={() => navigate("/dashboard")} className="flex items-center gap-3 group focus-ring rounded-xl cursor-pointer">
          <div className="bg-gradient-to-br from-sage-600 to-teal-50 p-2.5 rounded-xl shadow-glow group-hover:scale-105 transition-transform duration-300">
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-1">
        ...
        <NavItem to="/dashboard" icon={<BarChart3Icon className="w-5 h-5" />} label={t("nav.overview")} onNavigate={onNavigate} />
        <NavItem to="/patients" icon={<UsersIcon className="w-5 h-5" />} label={t("nav.patients")} onNavigate={onNavigate} />
        <NavItem to="/calendar" icon={<CalendarIcon className="w-5 h-5" />} label={t("nav.calendar")} onNavigate={onNavigate} />

        <SectionLabel>{t("nav.tools")}</SectionLabel>
        <NavItem to="/diet-generator" icon={<UtensilsIcon className="w-5 h-5" />} label={t("nav.diet_generator")} onNavigate={onNavigate} />
        <NavItem to="/metabolic-calculator" icon={<TargetIcon className="w-5 h-5" />} label={t("nav.metabolic_calculator")} onNavigate={onNavigate} />
        <NavItem to="/food-database" icon={<ClipboardListIcon className="w-5 h-5" />} label={t("nav.food_database")} onNavigate={onNavigate} />

        <SectionLabel>{t("nav.analysis")}</SectionLabel>
        <NavItem to="/reports" icon={<BarChart3Icon className="w-5 h-5" />} label={t("nav.reports")} onNavigate={onNavigate} />

        <SectionLabel>{t("nav.communication")}</SectionLabel>
        <NavItem to="/email-admin" icon={<PaperAirplaneIcon className="w-5 h-5" />} label={t("nav.send_plans")} onNavigate={onNavigate} />
      </nav>
