...
// Macronutrient distribution percentages by mode
const modeDistributions = {
  general: { protein: 25, carbs: 50, fat: 25 },
  clinical: { protein: 20, carbs: 50, fat: 30 },
  performance: { protein: 30, carbs: 45, fat: 25 },
  pediatric: { protein: 20, carbs: 55, fat: 25 },
  recovery: { protein: 35, carbs: 40, fat: 25 },
};
...
export const calculateBMR = ({
  ...
  height,
  age,
}: Omit<MetabolicData, "activityLevel">): number => {
  if (gender === "male") {
...
/**
 * Returns the default macro distribution for a given mode.
 */
export const getMacroDistributionForMode = (mode: string) => {
  return (
    modeDistributions[mode as keyof typeof modeDistributions] ||
    modeDistributions.general
  );
};
...
export const estimateTimeToGoal = (
  ...
  return i18next.t("metabolic.time_to_goal_weeks", {
    weeks: weeksToGoal.toFixed(1),
    defaultValue: `~${weeksToGoal.toFixed(1)} semanas`,
  });
};
